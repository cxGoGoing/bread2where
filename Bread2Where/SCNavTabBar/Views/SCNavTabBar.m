...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(10.0f, 6.0f, width - 20.0f, 23)];
    _line.backgroundColor = UIColorWithRGBA(41.0f, 152.0f, 167.0f, 1.0f);
    [_line makeCorner:12];
    [_navgationTabBar insertSubview:_line atIndex:0];
}
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        UIView * separetor = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(button.frame) + 5, 4, 2, button.size.height - 8)];
        separetor.backgroundColor = [UIColor darkGrayColor];
        [_items addObject:button];
//        [_items addObject:separetor];
